...
exports.default = strapi_1.factories.createCoreController('api::booking.booking', ({ strapi }) => ({
    ...
    async updateDetails(ctx) {
        const { documentId, phone, notes, country, city } = ctx.request.body;
        if (!documentId) {
        ...
        if (country)
            updateData.customerCountry = country;
        if (city)
            updateData.customerCity = city;
    ...
    async confirmPayment(ctx) {
        const { documentId, stripePaymentId } = ctx.request.body;
        if (!documentId) {
            return ctx.badRequest('documentId is required');
        }
        const updateData = {
